...
W,H = gen_decomposition(n,m,rank, state=random_state)
G, laplacian = gen_laplacian(size = m, radius = 0.30, state=random_state)
G, laplacian = gen_laplacian(size = m, radius = 0.30, state = random_state)
...
start = time.time()
G = gridSearcher(noise_D, laplacian = laplacian, algorithm = "diffusion", max_iter = 100000, tolerance = 1e-9, noise = noise_base, validate = 10, saver = "./analysis/testing_data/grid_search_" + str(rank) + "_" + str(beta) + "_geo.csv")
